...
詳細テストケース / 判定根拠の詳細品質は`test-case-design`、重大度付き独立レビューは`adversarial-review`を正本とします。
...
最終出力前に、実際に利用した入力が本Skillの入力契約を満たし、入力・仕様根拠・判断状態に停止条件へ該当する未解決状態がないか確認します。あわせて、生成した成果物へ本Skillの出力契約と既存の品質ゲートを再適用します。品質基準は本ガイダンスと各担当Skillの既存契約を正本とし、自己検証専用のrubricやチェックリストを別定義しません。
