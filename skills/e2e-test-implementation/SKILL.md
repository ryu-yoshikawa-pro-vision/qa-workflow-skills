...
2. 有効な`e2e-test-inspection`成果物または同等の確認済み情報と、詳細TC、明示E2E対象、既存E2E実装参照のいずれかを入力にします。実装前blockでは不足情報を理由としてコード変更を行わず、入力元の不足・block理由・次の担当を残します。
...
8. 実装対象は最低1件記録します。実装・再利用・拡張が成立した場合はE2E実装参照も最低1件記録し、実装前blockでは0件を許容します。E2E実装参照は、既存識別子またはrepo-relative test path + Playwright title pathで保持します。project、repeat、retryは実行単位情報として分離します。
