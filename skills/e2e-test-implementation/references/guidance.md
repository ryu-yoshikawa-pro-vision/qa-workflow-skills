...
入力元は詳細TC、ユーザー明示E2E対象、既存E2E実装参照のいずれかです。TCありの新規実装ではTC IDを入力元として扱うため、明示対象 / 既存E2E参照の出力欄を無条件に埋めません。TCなし経路では、明示E2E対象または既存E2E実装参照と確認済み期待挙動を直接利用します。TCを作る目的だけで`test-case-design`へ戻りません。期待挙動が確定できない範囲だけブロックし、コードを作成していない実装前blockでは架空のE2E実装参照を作らず、不足情報・block理由・次の担当を記録します。
