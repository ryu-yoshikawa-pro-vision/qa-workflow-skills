...
実装対象は最低1件必要です。TCなし経路でもE2E対象の識別子、入力となった明示対象 / 既存E2E参照、確認済み期待挙動を空欄にしません。
...
実装参照は最低1件必要です。repo-relative test fileとPlaywright title pathを用い、TCなし経路でもTC IDを創作せずに追跡可能にします。
