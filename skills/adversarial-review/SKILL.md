...
description: 仕様分析、テスト分析、テスト要求、テスト観点・条件、Coverage Item、ローレベルテストケース、カバレッジ分析を反証的にレビューし、誤り・抜け・過剰・根拠のない期待結果・追跡性欠陥を検出するSkill。成果物の最終品質確認や重大欠陥の洗い出しに使用する。
---
...
4. `Critical` / `Major` 指摘は利用前に明示的な処置を必要とします。
...
重大度、対象、問題、根拠、影響、推奨修正先、処置状態を持つ反証レビュー結果を作ります。
