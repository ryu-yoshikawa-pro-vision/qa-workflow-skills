...
def validate_current_structure_state(
    ...
    *,
    ...
    if not isinstance(state, dict) or set(state) != {"runtime_results", "carry_forward_entities"}:
    ...
    required_without_ci = {key for key in required_primary if key[1] != "ci"}
    projected_without_ci = {key for key in carry_ids if key[1] in primary_types and key[1] != "ci"}
    if projected_without_ci != required_without_ci:
    ...
    for row in carry:
        ...
        if row["entity_type"] in primary_types:
            if row["entity_type"] == "ci":
                ...
                if match is None or owner_tcn != match.group(1) or owner_tcn not in carried_tcn_ids or owner_model not in carried_model_keys:
    ...
    return {"runtime_results": canonicalize(raw_results), "carry_forward_entities": carry}
...
def verify_runtime_evidence(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if carry_rows and candidate_entity_map:
        try:
            ...
            issues.append({"issue_type": exc.issue_type, "blocking": True, "message": exc.message})
    current_structure_state = _current_structure_state_projection(skill, result_blocks, carry_rows)
