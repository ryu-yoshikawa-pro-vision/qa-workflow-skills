...
全体ワークフローでは次の9 Skillが同一Agent client上で利用可能であることを前提とします。
...
Agent Skills Specificationは共通Skill-to-Skill invocation APIを規定しません。本ワークフローは、Agent clientが必要なSkillを追加ロード / 利用できる実装で動作することを前提とします。
...
| 詳細テストケース / 判定根拠具体化 | `test-case-design` |
...
`qa-workflow`自身は、SPEC / DECISION / ASMの優先関係、version選択、情報源優先順位、仕様根拠の競合解消を行いません。
...
- テストケースの判定根拠 / 具体性判断が必要 → `test-case-design`
...
ユーザー、Project Context、または現在有効な対象範囲から外れる根拠がある場合に使用します。低プロダクトリスクだけを理由に使用しません。
...
- オーケストレーション以外の領域固有ロジックを再定義していない
...
最終出力前に、実際に利用した入力が本Skillの入力契約を満たし、入力・仕様根拠・判断状態に停止条件へ該当する未解決状態がないか確認します。あわせて、生成したオーケストレーション成果物へ本Skillの出力契約と既存の品質ゲートを再適用します。品質基準は本ガイダンスの既存定義を正本とし、自己検証専用のrubricやチェックリストを別定義しません。

確認対象は開始Skill、既存成果物の再利用、ブロック中 / 再開、変更伝播、修正ルーティング、ワークフロー完了判定など、本Skillが所有するオーケストレーション契約に限ります。他Skillの領域固有ロジックを再評価・再設計しません。

1. 実際に利用した入力が入力契約を満たし、停止条件へ該当する未解決状態がないか確認する
2. 生成したオーケストレーション成果物が出力契約と既存の品質ゲートを満たしているか確認する
3. 明白かつ局所的で、新しい領域判断を必要としないオーケストレーション契約違反だけを最大1回修正する
