...
from scripts.skills.evals.deterministic.common import CANONICAL_SKILLS, ID_PATTERNS, clean, nonempty_rows
...
def validate(text: str, expected: dict, eval_id: str) -> EvalResult:
    ...
    for row in additional:
        ...
        if need in {"必要", "ブロック中"} and owner != "e2e-test-execution":
            direct_execution.append(f"owner={owner}")
    ...
    actual_text = " ".join(row.get("値", "") for row in facts)
    result.add("E2E-AN-D007", not expected_ref or expected_ref in actual_text, "フィクスチャで必須のE2E実装参照が分析事実に存在すること", evidence={"expected": expected_ref} if expected_ref and expected_ref not in actual_text else None)
    ...
    if expected.get("additional_execution_required"):
        matching = [row for row in additional if clean(row.get("追加実行の必要性", "")) == "必要" and clean(row.get("実行担当", "")) == "e2e-test-execution" and clean(row.get("取得したい証拠", "")) and clean(row.get("検証する仮説", "")) and clean(row.get("必要な実行範囲", ""))]
        result.add("E2E-AN-D010", bool(matching), "追加実行に目的・仮説・範囲・execution routingがあること", evidence="追加実行要求なし" if not matching else None)
    return result
