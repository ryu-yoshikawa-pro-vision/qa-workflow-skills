...
4. runner管理のdependency / globalSetup / fixture / hook / webServer / teardownと、run外で確認済みの準備 / cleanupを区別し、二重実行しません。実行条件には`run外準備`行を必ず記録します。`setup / dependency / webServer / teardown`欄はraw設定、serverごとのownership・再利用・cleanupは専用ownership表を正本として記録します。`reuseExistingServer=true`は既存serverがあれば再利用し、なければ新しいserverを起動する設定値であり、actual reuseを示す事実ではありません。webServerなし、未確認 / 確認不能、確認済みの設定ありを区別し、設定ありならserver集合全件をownership表へ記録します。webServer未確認 / 確認不能ではrunnerを開始せず、今回runが所有していないprocessをcleanup目的で終了しません。run外cleanupを成功と記録する場合は、実行条件の`run外準備`を`実施`または`実施（詳細）`として独立記録します。automatic fixture、worker-scoped automatic fixture、worker再生成も副作用回数へ含めます。
