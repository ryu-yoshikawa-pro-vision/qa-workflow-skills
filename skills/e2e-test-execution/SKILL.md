...
7. terminal表示だけを正本にせず、既存machine-readable reporterまたは標準JSON等を安全に取得します。reporter / Playwright API / process観測が直接提供したraw factと、process exit code / 導出値を区別し、提供されないraw値を推測補完しません。preflightでブロックした場合はrunner未開始・artifact未生成を正当な結果として記録し、stale artifactを今回結果として利用しません。
