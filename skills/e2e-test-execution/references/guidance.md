...
1件の論理TCがproject、repeatEach、retry、parallel worker再生成で複数回操作され得ることを踏まえ、メール・削除・共有データ更新等の最大回数を決めます。副作用の許可範囲と最大回数は、自然言語から推測せず、空白付きの`/`または`／`をseparatorとする`<許可範囲> / <最大回数>`の構造で両方を記録します。scope内のpath / URLに含まれる裸のslashはseparatorにしません。対象URL / origin、実行入口、project、必要な認証・開始状態、許可範囲 / 最大回数、runner管理cleanup対象 / 方法、run外cleanup対象 / 方法をrunner開始前に確定できない場合は、その範囲をブロックします。テスト対象version / build IDは、実行自体が安全に成立する情報と分け、取得できない場合は`未確認`として結果解釈上の制約へ残せます。

## 準備とcleanup

runnerを開始する場合は、`run外準備`を実施または対象なし等の確認済み状態へ確定し、`setup / dependency / webServer / teardown`の4要素も空欄・未確認等を残しません。4要素は空白付きの`/`または`／`だけで分割し、各要素のpath / URL内の裸のslashは値として保持します。preflight blockでは未確認状態をブロック理由として記録できます。
