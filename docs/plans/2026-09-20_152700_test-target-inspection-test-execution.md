...
2. `test-target-inspection` は画面、領域、UI要素、表示状態、操作可能性、操作に対する反応、画面遷移、非同期状態、権限、データ依存等を記録します。DOM / accessibility tree等の構造情報だけでは視覚状態を確認できない場合、またはレイアウト、重なり、欠け、画像、canvas等の視覚情報が確認対象の場合は、screenshot等の画像も観測元として使用します。画像だけでroleやaccessible name等を推測せず、構造情報と視覚情報を必要に応じて併用します。後続比較に価値があり、現在の実行手段で安定したPlaywright ARIA snapshotを取得でき、保存時の機密情報保護と競合防止条件を満たす場合は、今回必要な画面 / 領域 / 状態に限定して`.aria.yml`等の機械可読な観測証跡として保存できます。全画面・全状態で必須化せず、永続比較用では操作用の一時refを含むAI向けsnapshotを正規証跡にしません。前回証跡との差分は変更候補の観測証拠として扱い、差分だけで仕様変更や下流成果物更新を確定しません。
...
8. `test-execution` のbrowser実行基盤はPlaywrightに固定し、TC開始前に`Playwright MCP → Playwright CLI → 独立した今回run用Playwright Libraryコード → 未実行 / ブロック中`の順で実行手段を判定します。前段の手段が利用可能で、TCに必要な操作・観測を本Skillの契約どおり実施できる場合は、その手段を使用し、速度・便利さ・成功しやすさを理由に下位手段へ切り替えません。Playwright CLIやPlaywright Library等が未導入の場合、本Skillのために新規installしません。独立一時コードは、MCP / CLIでは今回TCに必要な操作・観測を契約どおり表現できず、既存Playwright Libraryで実施可能な場合だけ使用します。`playwright test`等のrepo runnerを使う実行、将来も維持するrepo内E2Eコードの作成・更新は既存E2E Skillの責務を使います。視覚的な期待結果は現在選択しているPlaywright実行手段からscreenshot等を取得して確認し、視覚確認だけを理由に別実行手段へ切り替えません。実行手段の切替だけではTC入力snapshotを変更せず、未開始TCは切替後に開始状態を再確認し、開始済みTCは同じbrowser / sessionまたは判定に必要な状態継続を確認できる場合だけ継続します。確認できなければ`判定不能`として閉じ、再実行は別成果物 / versionとして`前回実行成果物参照 + 前回TC参照`で追跡します。元TC、案件コンテキスト、またはユーザーが明示したpreflightだけを使用し、TCで検証するUI操作をDOM、storage、cookie、network response、backend API / DB、アプリ内部状態等で代替してPASS条件を成立させません。
