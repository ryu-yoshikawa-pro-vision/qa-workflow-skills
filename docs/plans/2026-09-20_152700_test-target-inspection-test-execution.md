...
4. `test-target-inspection` の`assets/`は成果物テンプレートだけを保持します。案件固有のテスト対象資料を`qa-workflow-skills`リポジトリへ自動保存しません。永続化先はユーザーまたは対象案件が指定したパス / リポジトリを使用し、指定・書込能力がない場合は永続更新を実施済みと扱いません。永続保存・更新自体がユーザー要求に含まれる場合、保存成功またはユーザーによる保存不要への要求変更までは完了扱いにしません。`qa-workflow`利用時の案件コンテキスト`既存QA成果物`欄の更新は`qa-workflow`が担当し、`test-target-inspection`は保存済み成果物参照・範囲・鮮度を返します。新しいartifact registryは追加しません。
...
6. 現スコープの実行方式は`AI直接操作`と`自動実行`です。`AI直接操作`は利用可能なbrowser / computer操作能力で実対象UIを操作・観測できるTC、`自動実行`は既存Playwright E2Eを`e2e-test-execution`で実行できるTCを対象とします。API / DB等の専用実行方法やrunnerを本変更で新設しません。実行方式はTC単位で決め、同一成果物内で混在可能とします。混在時も`test-execution`が固定TC集合・方式・未実行TCを管理し、自動実行subsetだけを`e2e-test-execution`へ委譲してから同じ`test-execution`へ再開します。複数TCが同じlogical primaryへ対応する場合は委譲するlogical primary集合を一意化し、URL / project / 認証 / 開始状態 / テストデータ / 副作用・cleanup条件等を1 runで両立できない場合は自動実行subsetを複数の`e2e-test-execution`へ分けます。同じTCを複数方式で暗黙に二重実行しません。方式未指定時は、currentなE2E対応が存在するだけで`自動実行`を選ばず、そのE2EがTC判定に必要な期待結果を十分検証でき、今回の対象環境で安全に実行できる場合にだけ`自動実行`を優先します。それ以外でAI直接操作が安全に成立する場合は`AI直接操作`を使用します。今回新しい実行を要求されている場合は過去結果だけで代替しません。新しい汎用browser framework、runner、run registryは追加しません。
...
10. 今回要求されたTC集合と`test-execution`結果集合の完全性は、`test-execution`の`SKILL.md` / guidanceで定義する最終出力自己検証で確認します。`evals/deterministic/validator.py`は評価専用とし、runtime validatorとして呼び出しません。範囲指定で依頼された場合も、実行開始前に入力上の既存一意識別子から今回の具体的なTC集合へ解決して母集団を固定します。実行開始後はその集合を変更せず、ユーザーがTC追加・除外や方式変更を求めた場合は別の`test-execution`成果物 / versionとして扱います。複数TCを一意に識別できない場合は実行開始前に該当範囲をブロックします。既存`TC → E2E実装`対応の欠落・陳腐化は既存`coverage-analysis`（対象: `TC → E2E実装`）へ戻しますが、`coverage-analysis`へ新しい`TC → テスト実行結果`用途は追加しません。
