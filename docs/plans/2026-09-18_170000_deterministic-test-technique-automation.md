...
正規化済みモデルと各担当Skillが保存するcanonical machine Entityを意味上の正本とし、Machine Entityは`(skill, entity_type, entity_ref)`で識別します。Coverage表、生成組合せ、Coverage Item等の機械生成部分は派生成果物として扱います。既存成果物を再利用する場合も、runtime対象unitは現在のcanonical machine Entityと正規化済み入力からscriptを再実行し、保存済みruntime resultを現在世代の実行cacheとして扱いません。standalone最終検査はAgent / LLMが完成済みのexpected集合やdispatch stateを手組みせず、current Skillのcanonical normalized inputとcandidate artifactから`runtime_contract.py`が必須root runtimeを先に導出し、確認済みのcurrent structure / adapter parent runtime resultから条件付きdownstream runtimeを段階的に導出します。partial rerunでは同一成果物系列の直前artifact全文を追加入力とし、scope外Machine Entityを固定extract / 検証してcarry forward expectedへ使用します。上流Authority、直接依存する上流runtime結果、正規化済みモデル、runtime / generator contract、runtime / generator実装、静的参照データのいずれかが変わった場合は、fingerprint比較で影響する派生成果物をstaleとして`要再検証`へ戻し、再生成・再検証が完了するまで完了扱いしません。共通`runtime_contract.py`変更はそのhelperを使うunit全体の安全側の無効化として扱います。

scriptは仕様根拠や業務ルールを創作しません。`technique_slug`は正規テスト技法だけを表し、内部adapterは`model_type`だけを持ちます。adapterとCoverage childは同じ`condition_structure.py`実行で先にidentityを確定し、実際にCoverageを生成するchild modelが`selection_source / selection_key / technique_slug / derived_from_model_key`を保持します。dispatchは`model_type → generator`固定表だけを使います。Machine Entityは既存Skillが後続判断・再利用・stale判定に使う意味fieldを落とさず、CIには下流がgenerator内部modelを再読解しなくても具体手順へ展開できるcanonical `execution`またはsemantic Coverage Item本文を保存します。CIの完全IDは`TCN-\d{3}-CI\d{2,}`を正本とし、suffixは同一TCN内の採番比較だけに使います。canonicalizationはfingerprint計算だけでなくscriptが実際に処理する入力へ適用し、stable ID採番順も固定します。数値は対応範囲内でexactに処理し、state / flow / grammarの実行・導出順も一意にします。partial / unsupportedはclosure行の存在だけで完了扱いせず、currentな実Coverageまたは既存Skillで許可された扱いへ閉じていることを検査します。target Dispositionの`重複`はcycleを許さずcurrent CI / semantic CIへ終端させます。通常Coverage modelの`llm_fallback`は同じmodelのcurrent CIを根拠にします。Coverageを所有しないinternal adapterのpartial fallbackは影響するchild technique単位にunsupported itemを分けて同techniqueの直接定義Coverage model / CIへ閉じ、whole-model fallbackはactiveのまま残すselected child techniqueごとにcurrent Coverage model / CIまたは既存Disposition closureへ到達させます。最終完了判定では、既存`qa-workflow`が意味判断で確定したcanonical workflow scopeを境界とし、そのscope内でPython固定builderが期待runtime unit / Machine Entity集合をactual集合から独立したsourceから導出して実際集合と比較し、選択済みscope内の丸ごと欠落したunit / Entityを見逃しません。開始Skillや必要Skill自体の選択は従来どおり`qa-workflow`の意味判断であり、runtimeが自然言語要求から再推論しません。そのrouting妥当性はtrigger / semantic evalで検証します。standalone Skillでは`runtime_contract.py`の最終evidence確認がcurrent Skillのcanonical normalized inputから期待集合を段階的に導出し、集約stdin 16 MiB上限でcandidate artifactを検査します。

password、token、cookie、secret値そのものはcanonical runtime input / Machine Entity / TC machine evidenceへ保存せず、認証条件は取得方法・環境変数名等の非secret参照として扱います。期待結果が現在有効な仕様根拠へ追跡できない場合は、生成結果を完成済みテストとして扱わず、既存の停止条件・ブロック中・質問ルーティングへ戻します。
