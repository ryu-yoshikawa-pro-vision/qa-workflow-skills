...
8. `変更なし`または`削除確認`は、role / 権限、viewport、locale、feature flag、テストデータ、version / build等の差異が当該観測へ影響しないことを確認できる場合だけ使用する。比較条件を成立させられない場合は`未確認`または`確認不能`とする
...
| 副作用scope | 最大回数 | 準備回数 | 観測操作回数 | cleanup回数 | 累計実施回数 | cleanup対象 / 方法 | cleanup結果 | 残存状態 | 根拠 |
| --- | ---: | ---: | ---: | ---: | ---: | --- | --- | --- | --- |
...
新規成果物では全行を`追加`として重複列挙しません。`削除確認`では本体テーブルから対象行が消えるため、この表の確認条件、確認version / build、確認日時を必須にし、どの条件で削除を確認したかを保持します。
...
確認状態と更新区分を1つの状態値へ結合しません。`削除確認`は、対象が見えないことだけで確定せず、既存情報へ影響する確認条件が比較可能であり、条件差による非表示ではないことを確認できた場合だけ使用します。
...
実対象のデータ・設定・権限・外部送信等を変更する、またはcleanupを要する観測では、対象origin、許可scope、最大回数、cleanup方法を確認します。最終的に元へ戻す変更も同じ安全契約で扱います。準備、観測操作、cleanupの各状態変更を対応する副作用scopeへ所属させ、scope単位で累計します。cleanupが同じscopeを消費する場合は、観測操作を開始する前に必要なcleanupまで実施できる残数を確認します。確認できなければ状態変更操作を行いません。実施した場合は工程別実施回数、累計実施回数、cleanup結果、残存状態を`副作用・cleanup`へ記録します。必要なcleanupが`失敗 / 未確認 / 一部失敗`、または許可されていない残存状態がある場合は対応範囲を完了扱いにしません。
