...
永続的な副作用を伴う観測を行った場合だけ記録します。

| 副作用scope | 最大回数 | 実施回数 | cleanup対象 / 方法 | cleanup結果 | 残存状態 | 根拠 |
| --- | ---: | ---: | --- | --- | --- | --- |
...
永続的な副作用が必要な観測では、対象origin、許可scope、最大回数、cleanup方法を確認します。確認できなければ操作しません。実施した場合はscope単位で実施回数を累計し、cleanup結果と残存状態を`副作用・cleanup`へ記録します。
