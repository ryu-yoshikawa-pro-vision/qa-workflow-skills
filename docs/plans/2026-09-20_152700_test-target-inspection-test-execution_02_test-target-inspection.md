...
6. 既存資料と今回確認事実を比較する
7. 変更がある範囲だけ更新候補を作成し、未確認範囲を推測で書き換えない
8. 各情報が確認元と確認日時 / revisionへ辿れるようにし、今回未再確認の既存情報を今回確認済みへ更新しない
9. 更新候補へ出力契約・参照整合を適用する
10. 永続更新を要求され、保存先と書込能力があり、更新候補が契約を満たす場合だけ対象成果物へ保存する
11. 保存後に読取可能なら再読込し、保存内容が更新候補と一致することを確認する
12. `qa-workflow`の案件コンテキストを利用している場合は、保存済み成果物の場所・範囲・鮮度を既存`既存QA成果物`欄へ反映する
...
各表の`確認状態`は、少なくとも`今回確認 / 既存資料から継承・未再確認 / 未確認 / 確認不能`を区別できる契約にします。部分更新で成果物全体の確認日時だけを更新し、未再確認の行までcurrentに見せません。
...
| 操作元 | 操作 | 条件 | 遷移先 / 観測結果 | 確認状態 | 確認元 | 確認日時 / revision |
| --- | --- | --- | --- | --- | --- | --- |

### データ・権限依存

| 対象 | 条件 | 影響 | 確認元 | 確認状態 | 確認日時 / revision |
| --- | --- | --- | --- | --- | --- |

### 既存テスト実装との対応

| 対象 / 要素 | 種別 | repo参照 | 関係 | 確認状態 | 確認日時 / revision |
| --- | --- | --- | --- | --- | --- |
