...
- `test_analysis_context.authority_refs[] / risk_refs[]`をdependency identityとして検証する。artifact modeでAuthorityをcurrent Entityへ解決し、direct modeでも各Authority refを渡されたcurrent Authority Entityへ解決してそのEntityだけを使う。両modeのunknown Authority、unknown Risk、duplicate ref、およびartifact modeのmissing Authorityを拒否し、caller fingerprintを受け取らない。参照Risk / Authorityのcontent変更でcontextだけをstaleにし、未参照Entityの変更ではstaleにしない
...
- node / edge type
- `change_kind`を既存`新規 / 変更 / 削除 / 回帰影響 / 参考`またはnullに限定し、`expected_impact`とともに探索ロジックで創作・欠落させずMachine Entityへ保持する
...
- `pcg32-v1`のstate / seeding / XSH RRと、`threshold = (2^32 - bound) mod bound`、`r < threshold`をreject、`r mod bound`を返すbounded integer規則
- seed=42固定raw output vector
- seed=42 / bound=10でthreshold=6、最初の4結果`3, 7, 4, 5`
- seed=42 / bound=2147483649でthreshold=2147483647、最初の4結果`559678134, 974992175, 64156306, 1067743306`。2件目でraw `2068313097`をrejectして`3122475824`をacceptすることを固定検証し、単純moduloや別rejection方式を検出する
- bound=`2^32`でthreshold=0となりraw uint32をそのまま返す
- 同seedで同列
- seed差
...
- JSON pathはruntime-v1文法`$ (.<identifier>|[<index>])*`だけを許可し、identifier=`[A-Za-z_][A-Za-z0-9_-]*`、index=`0|[1-9][0-9]*`を検証する
- `$`, `$.amount`, `$.items[0].price`, `$[0]`をacceptし、quoted / bracket string key、escape、wildcard、filter、slice、recursive descent、不正index、表現不能object keyを`unsupported_path`にする。transform `path`と`output_path`へ同じparserを使う
