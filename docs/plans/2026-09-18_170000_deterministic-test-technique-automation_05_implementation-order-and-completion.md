...
- `qa-workflow`を経由しないstandalone Skillでは、最終成果物の`runtime_contract.py verify_runtime_evidence`へcanonical normalized inputとcandidate全文を渡す。test-analysis / coverage-analysis / qa-workflowはpreviousをnull固定とし、TRD / TCD / test-case-designだけ固定helperがcarry-forwardを必要と判断した時にprevious全文を渡す。root期待runtimeとMachine Entity集合を固定導出し、`valid=false`なら完成扱いしない
...
- model / artifact全runtime unitの`Machine Runtime Input / Result`を決定論的に抽出し、strict decode、fingerprint一致、同条件でのround-trip再実行が成立する。workflow再利用では保存済みresultをcurrent cacheとして採用せず現在scriptを再実行する。保存`runtime_units[]`はevidence identityとcurrent resultとの照合に使い、completion / closure / unsupported等の意味状態は`current_runtime_units[]`を正本にする。同じgenerationでも保存result projection差分を検出し、current扱いしない。model resultと集約runtime rowは`runtime_contract.py`固定projectionから作り、`upstream_entity_fingerprints[]`や`freshness_status`をcaller / LLMが補完しない
- runtime対象Skillのstandalone最終出力で`runtime_contract.py verify_runtime_evidence`がcanonical normalized Skill inputからroot期待runtime unitを先に導出し、確認済みcurrent parent resultから条件付きdownstream期待runtimeを段階的に導出してcandidate artifactのruntime blockとMachine Entityのmissing / extra / incomplete / duplicateをproduction経路で検出できる。TRD / TCD / test-case-designは固定boolean `partial_rerun`でfull buildとpartial rerunを区別し、partial rerunではscope外primary IDがなくDispositionだけを維持する場合も同一成果物系列の直前artifact全文を固定extract / 検証する。Agent / LLMから完成済みexpected集合、dispatch state、手組みのprevious Entity配列を受けず、root欠落を理由に期待集合を空へ縮退しない。helper operation自身をruntime unitへ数えない
...
- `workflow_runtime.py`はcanonical `workflow_scopes[]`に本Planruntime対象scopeが1件以上ある場合だけdispatchし、scope内のmodel / artifact両runtime unit、Machine Entityのsemantic / runtime dependency、upstream Entity内容変更、`(skill, runtime_unit_key)` dependency、同一`runtime_contract.py` fixed builderがscope sourceから内部導出したexpected runtime / Entity集合のmissing / extra、dependency missing / duplicate / cycle、generation / implementation変更、stale、current rerun由来`current_runtime_units[]`のmaterialize `model_completion[] / target_mappings[] / target_dispositions[]`によるCoverage所有model単位の完了、target`重複`graph、partial / whole-model unsupported closure、局所ブロック、legacyを処理できる。保存`runtime_units[]`とcurrent rerun projection / result fingerprintの差はstaleとして完了を阻止する。Agent / LLMから完成済みexpected集合を受けず、自身を`runtime_units[] / current_runtime_units[]`と内部expected集合へ含めない。self inclusionを`invalid_input`にする。本Planruntime対象scopeが0件のE2E-only経路ではdispatchせず既存`qa-workflow`契約を維持する。scope選択自体の意味妥当性は既存routing / trigger / semantic evalで確認する。`can_complete`は本Planruntime範囲の必要条件であり、混在workflowでは既存E2E等の完了条件を別途満たすまでworkflow全体を完了にしない
- `traceability.py`と`workflow_runtime.py`が同じ`runtime_contract.py` freshness / closure関数から同じruntime / Entity freshnessとmodel closureを得る。両者は保存runtime evidenceとcurrent script rerun結果をresult projection / fingerprintまで照合し、意味上のcompletion / closure判定にはcurrent rerunを使う。workflow_runtime resultをtraceabilityの依存入力にせず、traceability自身とworkflow_runtime自身をtraceabilityのfreshness入力runtime集合へ含めない
