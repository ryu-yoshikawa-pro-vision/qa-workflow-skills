...
`WF-D006`、`WF-D007`、`WF-D008`、`WF-D014`、fixture期待値、辞書化・集合化等を含め、Skill名だけへ情報を縮約する実装を確認します。複数対象を区別しなければ誤判定する箇所は`(Skill, 対象 / 実行範囲)`単位で検証できるようにします。

ただし、単に「そのSkillを少なくとも一度利用したか」を確認するだけで十分なassertionまで機械的に複雑化しません。開始対象・最終対象・期待する工程順序など、対象を区別しないと誤PASSする契約だけを対象-awareにします。
...
- 構造化された各test / attemptの実行結果
- 実行対象ごとの結果または未実行理由
- reporter / artifactの取得方法と、安全確認したoutput先
- 利用可能な証跡参照
- cleanup結果
- 残存副作用
- ブロック中
...
- 条件分岐型の代表経路へ更新
- `Skill + 対象 / 実行範囲`状態へ変更
- `WF-D006`、`WF-D007`、`WF-D008`、`WF-D012`、`WF-D014`等について、Skill名だけへの辞書化・集合化・比較で対象を失う箇所を確認し、必要なものだけtarget-awareに修正
...
- `WF-D006`、`WF-D007`、`WF-D008`、`WF-D012`、`WF-D014`等について、複数対象を区別しないことで誤判定するSkill単独比較が解消される
- 既存QA IDグラフをE2E参照のためだけに拡張せず、TC ↔ E2E実装参照を別の対応関係として決定論的に検証できる
