...
単一用途のSkillへ意味のない対象名を増やしません。対象を区別する必要がなければ空欄を許容します。

状態・validatorの最低契約は次とします。

- `WF-D006`: `expected_start_skill`は維持し、開始対象を区別するfixtureでは`expected_start_target`も比較する
- `WF-D007`: `expected_final_skill`は維持し、最終対象を区別するfixtureでは`expected_final_target`も比較する
...
E2E対象を扱う`coverage-analysis`成果物では、最低限次の対応表を持ちます。
...
新規実装・既存再利用・既存拡張では、対象repoに既存の追跡方式がなければrepo-relative test path + title path等でE2E実装を一意に辿れるようにします。E2E対象外・ブロック中では存在しない実装参照を要求しません。決定論的評価はこの対応表を既存QA IDグラフとは別契約として検証します。
...
- 対象testがsnapshotを利用する場合のsnapshot生成・更新設定、snapshot保存先、source側への書き込み可能性
...
全repoを再監査せず、今回の実行経路から到達する設定・setup・teardown・fixture等へ確認範囲を限定します。Playwrightの機能や設定の対応状況・挙動に不確実性がある場合だけ、その時点で対象repoの実効設定とPlaywright公式仕様を確認します。Playwrightのversion確認を毎回の必須手順にはしません。
...
これらはPlaywright run自体によって起動します。executionが同じ準備処理をrunの外側で重複実行しません。
...
`未実行`はPlaywrightの`TestResult.status`ではなく、ワークフロー上「要求されたprimary testだが実行されなかった」状態として分けます。`実行対象 → 結果または未実行理由`の完全性判定は要求されたprimary testを母集団とし、dependency / teardown testを要求件数へ混ぜません。
...
これらはPlaywright runの一部として実行されます。runner管理のcleanupが完了している場合、executionが同じcleanupをrunの外側でもう一度実行しません。
...
- テスト対象version
- Playwright run全体status / process exit code / run-level error
- 構造化された各test / attemptの実行結果。primary / dependency / teardownを区別
- 要求されたprimary testごとの結果または未実行理由
...
- `expected_start_target` / `expected_final_target` / `expected_scoped_skill_states`等、対象別期待値を表現できるfixture契約を追加し、対象を失って誤PASSする回帰を検出
...
- run全体status / process exit code / run-level error保持
- primary / dependency / teardown testを区別し、生の`TestResult.status` / `outcome` / attemptsを保持
- runner管理teardownとrun外cleanupを分離し、二重cleanupを禁止したうえでcleanup結果を記録
- 証跡・secret保護
...
- executionが直接開始時でもproject dependencies、setup / teardown、URL、副作用、parallel / retry、test本体から到達する安全判断上必要な経路を確認できる
- runner管理のsetup / cleanupとrun外でexecutionが明示実行する処理を区別し、同じ処理を二重実行しない
- executionが必要なrun外準備だけ確認済み方法で実施し、runner管理分を含むcleanup結果を確認できる
- executionが構造化されたPlaywright結果を取得し、run全体status / process exit code / run-level errorと、attempt / retry / outcomeを失わない
- 構造化resultが今回runで生成・更新されたことを確認し、古いartifactを今回結果として利用しない
- 要求されたprimary testとdependency / teardown testを区別し、primary testごとに結果または未実行理由が存在する
...
- 「実行結果だけ」の依頼では原因分析を勝手に追加せず、それ以外の分析対象異常を`e2e-test-result-analysis`へ渡せる
- FAILとプロダクト不具合を区別できる
- 分析結果から最も早い責任工程へ戻し、必要範囲だけ再検証・再実行できる
- ワークフロー完了と全E2E PASSを区別できる
- `Skill + 対象 / 実行範囲`で同一Skillの複数利用を状態管理でき、複数用途Skillの正規対象値が固定される
- `WF-D006` / `WF-D007`は必要なfixtureだけtarget-aware、`WF-D008`はSkill単位を維持、`WF-D012` / `WF-D014`は対象別に検証できる
- 既存QA IDグラフをE2E参照のためだけに拡張せず、`TC ID | 扱い | E2E実装参照 | 根拠 / 備考`の対応関係として決定論的に検証できる
