...
- inputはstrict JSON `{operation:"verify_runtime_evidence", skill, normalized_skill_input, artifact_markdown, previous_machine_entities}`。`previous_machine_entities`は通常null、partial rerunでscope外Entityを引き継ぐ場合だけ配列を許可する。同配列は前回成果物の`Machine Entities` blockを同じ`runtime_contract.py`の固定extract / strict decode / fingerprint検証で得たrowだけを渡し、Agent / LLMが手組み・削減・改変しない。このoperationは最終成果物全体を扱う集約処理として扱い、stdin hard limitは16 MiBとする。上限はJSON escape後の実際のUTF-8 stdin bytesへ適用し、超過時はtruncateせず`limit_exceeded`として成果物を完成扱いしない。runtime-v1では16 MiBを超えるstandalone成果物の最終evidence確認をサポートしない
