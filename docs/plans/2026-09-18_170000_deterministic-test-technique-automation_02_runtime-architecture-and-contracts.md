...
`workflow_runtime.py`は工程固有の意味判断を行いません。保存artifactの`runtime_units[]`は必要なevidence identityと保存結果の照合に使用し、現在のcanonical inputでscriptを再実行した`current_runtime_units[]`をfreshness、closure、materialize completion、unsupported状態など現在の意味状態の正本にします。開始Skill、必要Skill、対象 / 実行範囲、意味上の変更影響、既存成果物を意味的に再利用できるかの判断は既存`qa-workflow`責務に残し、その決定をcanonical workflow scopeとして`workflow_runtime.py`へ渡します。runtimeの完全性保証はこのscope内を対象とし、scope自体の意味的な正しさは既存routing契約、trigger eval、semantic evalで検証します。

### standalone最終runtime evidence確認

`qa-workflow`を経由しないSkillでも必須runtime unitまたはMachine Entityの丸ごと省略をproduction経路で検出するため、各Skill-local `runtime_contract.py`へ同一実装の`verify_runtime_evidence` operationを持たせます。これはdispatch対象runtime unitではなく、最終出力前の固定検査です。

- inputはstrict JSON `{operation:"verify_runtime_evidence", skill, normalized_skill_input, artifact_markdown, previous_artifact_markdown, partial_rerun}`。`partial_rerun`はfull build / partial rerunを示す必須booleanで、Agentがscopeから推測した別flagを渡さない。test-requirement-design / test-condition-design / test-case-designではfull buildを`false + previous=null`、partial rerunを`true + 同一成果物系列の直前artifact全文`とする。partial rerunではscope外Dispositionだけが残る場合もprevious全文を必須とする。test-analysis / coverage-analysis / qa-workflowでは`false + null`だけを許可し、自Skill Entityのprevious carry-forwardを行わない。Agent / LLMが前回Machine Entityだけを抜き出した配列を手組みして渡す経路は作らない。同じ`runtime_contract.py`が前回成果物のMachine Entity blockとfull ID / mapping stateを固定extract / strict decodeし、identity一意性、entity type別schema、`content_fingerprint`再計算、scope外active stateとの対応を検証する。検証できない前回成果物は`invalid_previous_artifact`として`valid=false`にし、carry forwardへ使わない。このoperationは最終成果物全体を扱う集約処理として扱い、candidateとpreviousを含むJSON escape後の実際のUTF-8 stdin全体へ16 MiB hard limitを適用し、超過時はtruncateせず`limit_exceeded`として成果物を完成扱いしない
...
- partial carry-forward freshnessの再帰境界は[部分更新時のMachine Entity契約](./2026-09-18_170000_deterministic-test-technique-automation_02_identity-materialize-and-workflow-contracts.md#712-部分更新時のmachine-entity)に従う。standalone verifierはcross-Skill upstreamのcurrent identity / `content_fingerprint`を確認し、workflow / traceabilityの全体freshness評価は従来どおり全Entity graphを再帰する
...
- `verify_runtime_evidence`のtop-level `artifact_markdown` / `previous_artifact_markdown`は成果物全文transportのため1文字列64 KiB上限の対象外とする。ただし、両artifactとその他fieldを含む実際のUTF-8 stdin全体の16 MiB上限は維持する。artifactから抽出したMachine Runtime Input / Result / Machine Entities等のJSONには通常の64 KiB文字列上限を適用する
...
- expected / actual runtimeのmissing、extra、Inputだけ、Resultだけ、duplicateを検出し、1件でもあれば`valid=false`とする。actual identity集合からexpectedを逆算しない
- 同じhelperはnormalized source、検証済みcurrent structure / materialize result、partial rerun時に`previous_artifact_markdown`から検証済みとして抽出したscope外Machine Entityから`expected_entities[]`を固定導出する。`update_scope_*`内はcurrent resultを正本にし、scope外activeは[identity・materialize契約](./2026-09-18_170000_deterministic-test-technique-automation_02_identity-materialize-and-workflow-contracts.md) §7.1.2のcarry-forward集合だけを期待する。candidate artifactの`Machine Entities` blockをstrict decodeし、identity集合のmissing / extra / duplicate、entity type別schema、`content_fingerprint`再計算一致を検査する。candidate artifactのactual Entity集合またはcallerが組み立てたprevious Entity配列からexpectedを逆算しない
- outputは`{valid, issues[], expected_runtime_units[], actual_runtime_units[], missing[], extra[], incomplete_pairs[], duplicates[], expected_entities[], actual_entities[], missing_entities[], extra_entities[], duplicate_entities[], current_structure_state}`の固定JSONとする。`valid=true`では`current_structure_state={runtime_results:[...], carry_forward_entities:[...], previous_ci_id_state:[...]}`を必ず返す。`previous_ci_id_state`はtest-condition-designでstrict検証済みprevious artifactのmaterialize result群から統合した全CI stateであり、他Skillでは空配列とする。`valid=false`とCLI errorでは`current_structure_state=null`とする。成功・失敗でfieldの有無を変えない
- current_structure_stateはこのverifierだけがstrict検証後に生成する固定projectionである。primary carry-forward identityはscope外active identityとexact matchさせ、scope内 / unknown row、owner不明row、producer schemaに合わないDispositionを拒否する。test-condition-designではscope外CIのexact identity集合を検証できるよう、previous artifactのTCN別materialize `ci_id_state[]`をstrictに統合した`previous_ci_id_state[]`も渡す。workflow / traceabilityへ渡す時もAgent / LLMはstateを再構成・編集しない
...
- qa-workflow / coverage-analysisでは各scope担当Skillのverifierを先に実行し、`valid=true`の返却`current_structure_state`を変更せずworkflow_scopes[] / analysis_scopes[]へ渡す。これをworkflow全体の完了判定やtraceability inputの手組みで代替しない
...
- `traceability.py / workflow_runtime.py`へ渡す`runtime_unit_row`はcurrent envelopeから`skill / runtime_unit_key / model_key / support_status / result_status / runtime_status / runtime_required / deterministic_generated / generation_fingerprint / upstream_entity_fingerprints[] / upstream_runtime_units[] / unsupported_items[] / runtime_contract_version / generator_contract_version / runtime_implementation_fingerprint / generator_implementation_fingerprint / static_data_versions`を固定転記する。またcanonical full Machine Runtime Result envelopeのSHA-256 `result_fingerprint`を固定builderが計算する。`model_completion[] / target_mappings[] / target_dispositions[]`は`materialize_coverage.py` unitだけcurrent materialize resultから追加し、他unitでは空配列固定とする
- これらのprojectionは保存用`Machine Runtime Result`と同じsource envelopeから作り、field名の別解釈や匿名の中間schemaを作らない
- 保存側`runtime_units[]`とcurrent rerun由来`current_runtime_units[]`はidentity / generationだけでなく`runtime_unit_row`のresult projectionと`result_fingerprint`を一致検証する。generationが同一でも`payload / issues / model_completion[] / target_mappings[] / target_dispositions[] / unsupported_items[]`等が異なる保存rowはcurrent扱いしない。`generation_fingerprint`は既存どおりinput / contract / implementation / upstream / static data由来とし、result内容を混ぜない
...
`stale`は`result_status`ではありません。semantic dependency preflightに成功した現在inputを現在scriptで正常実行したruntime resultは、保存時に`freshness_status=current`とします。`workflow_runtime.py`は保存済みdependency / generation / result projectionをcurrent rerunと共通freshness関数で再検証し、差分があれば`stale`へ変更して下流へ伝播します。Closure、materialize completion、unsupported closure等の現在状態判断には`current_runtime_units[]`だけを使い、保存側projectionをcurrent cacheとして使いません。`qa-workflow`だけをfreshnessの唯一の付与主体にせず、実行直後のcurrent判定と最終集約を分離します。
...
`runtime_contract.py`全体が`runtime_implementation_fingerprint`の対象です。このため、同一Skillの`runtime_contract.py`を変更すると、そのhelperを使う当該Skillのruntime unit generationは変わり、runtime evidenceは現在scriptで再実行します。canonical / Machine Entity helperはruntime対象6 Skillと`spec-analysis`へ同一内容で同期します。Entity freshnessでは、このaggregate runtime generationに加えて、Entity単位runtime dependency projectionを使い、別scope更新だけで内容不変のscope外Entityをstaleにしない境界を設けます。このprojectionはruntime unit generationを局所化せず、runtime unitのdispatch identityやgeneration fingerprintを変更しません。
...
- `runtime_dependencies[]`はそのEntityの現在状態を成立させるruntime unitを列挙する。通常はruntime unit identityを保存する。Entityを生成するaggregate structure / materialize runtimeでは、scope外Entity freshness境界を保つため固定projection key `<base_runtime_unit_key>#entity:<digest>`を保存できる。`digest`は`machine_entity_runtime_unit_key()`がMachine Entity identity `{skill, entity_type, entity_ref}`のcanonical JSONから計算するfull SHA-256 (`sha256:<64 lowercase hex>`)であり、短縮しない
- `#entity:<digest>`付きkeyはMachine Entity freshness metadata専用で、dispatch対象runtime unitではない。`expected_runtime_units[]`、`runtime_units[]`、`current_runtime_units[]`またはruntime dependency graphのnodeへ追加しない
- freshness評価では`#entity:<digest>`を除いたbase runtime unitを`current_runtime_units`から解決し、`runtime_unit_row`が保持するcurrent producer metadataとEntity自身からscoped generationを再計算して保存値と比較する。scoped generationのcanonical材料はschema `machine-entity-runtime-generation-v1`、Entity identity、`content_fingerprint`、`runtime_contract_version`、`generator_contract_version`、`runtime_implementation_fingerprint`、`generator_implementation_fingerprint`、`static_data_versions`。`upstream_entity_dependencies[]`は既存のsemantic freshness規則で別に評価し、scoped generationへ重複して埋め込まない
- Authority等、上流もruntimeも持たないsource Entityでは両配列を空にできる
...
- test-analysis context: `{scope, objectives[], test_levels[], environment_constraints[], exclusions[], blockers[], test_focus_items[], testability_decisions[], residual_risks[], authority_refs[], risk_refs[]}`。`authority_refs[]`は判断で実際に参照したAuthority Machine Entityの`entity_ref`、`risk_refs[]`は同一分析で生成したProduct Riskの`risk_id`を保持する。どちらも重複不可・文字列配列・昇順canonicalとし、自然言語fieldからreferenceを推測しない
...
- test-analysis context: `content.authority_refs[]`で明示されたAuthorityと、`content.risk_refs[]`で明示された同一invocation内Product Risk。artifact modeではAuthorityをcurrent Machine Entityへ完全解決し、参照先がなければ正常完了にしない。direct modeでも各Authority refを渡されたcurrent Authority Machine Entityへ解決し、そのEntityだけをdependencyにする。どちらのmodeもunknown Authority refは拒否し、Entityを合成しない。Product Riskは同一invocationで生成したEntityへ完全解決し、そのcanonical `content_fingerprint`を保存する。unknown Risk / duplicate refは拒否し、自然言語fieldやcaller提供fingerprintからdependencyを作らない
...
1文字列64 KiB上限は通常のJSON stringに適用する。例外は`verify_runtime_evidence` requestのtop-level `artifact_markdown`と`previous_artifact_markdown`だけであり、この2 fieldは除外する。`normalized_skill_input`、他のrequest field、およびartifactから抽出したMachine Runtime Input / Result / Machine Entities等のJSON stringには通常の64 KiB上限を適用する。artifact全文は候補・previousを合わせたrequest全体の16 MiB上限で制御する。
