...
- current_structure_stateはこのverifierだけがstrict検証後に生成する固定projectionである。primary carry-forward identityはscope外active identityとexact matchさせ、scope内 / unknown row、owner不明row、producer schemaに合わないDispositionを拒否する。workflow / traceabilityへ渡す時もAgent / LLMはstateを再構成・編集しない
