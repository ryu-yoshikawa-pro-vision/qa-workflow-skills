...
- inputはstrict JSON `{operation:"verify_runtime_evidence", skill, normalized_skill_input, artifact_markdown, previous_artifact_markdown}`。`previous_artifact_markdown`は通常null、partial rerunでscope外Entityを引き継ぐ場合だけ同一成果物系列の直前成果物全文を許可する。Agent / LLMが前回Machine Entityだけを抜き出した配列を手組みして渡す経路は作らない。同じ`runtime_contract.py`が前回成果物のMachine Entity blockとfull ID / mapping stateを固定extract / strict decodeし、identity一意性、entity type別schema、`content_fingerprint`再計算、scope外active stateとの対応を検証する。検証できない前回成果物は`invalid_previous_artifact`として`valid=false`にし、carry forwardへ使わない。このoperationは最終成果物全体を扱う集約処理として扱い、candidateとpreviousを含むJSON escape後の実際のUTF-8 stdin全体へ16 MiB hard limitを適用し、超過時はtruncateせず`limit_exceeded`として成果物を完成扱いしない
- previous_artifact_markdownのlifecycleはSkillごとに固定する。test-analysis / coverage-analysis / qa-workflowはruntime-v1で自Skill所有Entityをcarry forwardしないため常にnullとし、非nullはinvalid_previous_artifactにする。test-requirement-design / test-condition-design / test-case-designは固定helperがscope外active Entityのcarry-forwardを必要と判定した場合だけ同一成果物系列の直前artifact全文を許可し、その必要がない場合はnullを要求する。
