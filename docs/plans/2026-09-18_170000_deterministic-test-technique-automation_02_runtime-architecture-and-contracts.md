...
- outputは`{valid, issues[], expected_runtime_units[], actual_runtime_units[], missing[], extra[], incomplete_pairs[], duplicates[], expected_entities[], actual_entities[], missing_entities[], extra_entities[], duplicate_entities[]}`の固定JSONとする。正常時の`issues[]`は空配列とする
