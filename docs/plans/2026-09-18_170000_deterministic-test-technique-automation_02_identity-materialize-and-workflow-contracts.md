...
- `previous_artifact_markdown=null`を許可できるのは、固定されたprevious full ID stateと`update_scope_*`からscope外active Entityのcarry-forwardが不要と判定できる場合だけとする。scope外active IDがあり、そのcanonical Machine Entity rowを検証して維持する必要があるpartial rerunでは、同一成果物系列の直前artifact全文をproduction validatorが必須化する。previousがないことを理由にscope外Entityをexpected集合から落とさない
