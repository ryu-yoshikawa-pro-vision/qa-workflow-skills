...
- `partial_rerun`はverify requestの必須booleanとする。TRD / TCD / test-case-designのfull buildは`false + previous_artifact_markdown=null`、partial rerunはscope外primary ID / Dispositionの有無にかかわらず`true + 同一成果物系列の直前artifact全文`とする。Dispositionはprimary ID stateを持たないため、partial rerun全体でprevious artifactをstrict検証し、scope外Dispositionの欠落を防ぐ。test-analysis / coverage-analysis / qa-workflowはruntime-v1で`false + null`固定とする。legacy初回昇格はpartial rerunではなく、全active legacy rowをupdate scopeへ含めてnormal full snapshotを成立させる。previous欠落を理由にscope外Entityをexpected集合から落とさない
- fixed helperはstrict検証済みscope外Machine Entity rowを`current_structure_state.carry_forward_entities[]`として固定projectionへ出せる。このprojectionはcanonical row全体を保持し、identity / ownership / `content` / fingerprint / semantic dependency / runtime dependencyを書き換えない。workflowとtraceabilityはcandidateの`current_entities[]`からExpectedを逆算せず、standaloneと同じ`_expected_entities()`およびcarry-forward helperで、current runtime result Entityとこのprojectionを統合する。partial scopeにactive out-of-scope IDがあるのにprojectionがない、または固定projectionからIDを満たせない場合は完了させない
- `current_structure_state`は`verify_runtime_evidence`だけがfixed builderから生成する。workflow / traceabilityは返却projectionを無変更で受け、共通validatorでschema、runtime result identity、Machine Entity schema / fingerprint、primary identity exact match、scope-in混入、ownerとproducer ownershipを検証する。test-condition-designの`previous_ci_id_state[]`はstrict検証済みprevious artifactのTCN別materialize resultからverifierが固定生成し、scope外CIのprimary identityもこのstateとのexact matchを要求する。callerがcarry projectionへrowを加えてexpected集合を広げることはできない
- Dispositionのscopeは実producerのupstream schemaで決める。test-requirement-designはAuthority / Product Risk、test-condition-designのrequirement DispositionはTR、test-case-designはTCN / CI / environment requirement / test data requirementだけをupstreamにする。normalized current upstreamはcurrent resultを正本としてprevious rowを再利用・復活させず、scope外upstreamの検証済みrowだけをcarry forwardする。unknown ownerや不正typeは拒否する
- previous full CI stateは`artifact:materialize_coverage:<TCN ID>`ごとの`ci_id_state[]`から統合する。各rowのCI prefixはproducer TCNと一致し、CI identityは成果物系列内で一意であることを検証する。materialize invocationへ渡す`previous_ci_ids[]`はそのcurrent `tcn_id`のsubsetだけを比較し、他TCN分はprevious artifactの統合stateからcarry forwardする
- scope外Entityが依存するaggregate producer runtime unitのgenerationが別scope更新によって変わっただけでは、そのEntityを自動staleにしない。Entity単位runtime dependency projectionがcurrent Entity contentとproducer contract / implementation / static dataを検証する。Entity自身のcontent、semantic dependency、producer implementation / contract / static dataが変わった場合はstaleにし、`要再検証` / blockerへ戻す。runtime unit自体のfreshness・generation検証を弱めない
- standalone `verify_runtime_evidence`のcarry-forward freshnessは、carry-forward対象の当該Skill所有Entityをrootとして同Skill所有Entity graphまで再帰する。carry-forward Entity自身のproducer runtime dependencyと、同Skill upstream Entityのidentity / `content_fingerprint` / producer freshnessを検証する。cross-Skill upstream Entityはcandidate内のcurrent Entityとのidentityおよび`content_fingerprint`一致までをdownstream Skill verifierの境界とし、そのEntity自身の`runtime_dependencies[]`へは再帰しない。cross-Skill producer freshnessはproducer Skill自身のverification、またはqa-workflow / traceabilityの全体freshness検証で保証する
...
各Skillは同じ正規化済みsourceから`expected_entity_identities[]`を固定builderで生成し、Machine Entity actual rowの存在を入力にして期待identityを逆算しません。Skill-local validatorでは同じfixed builder出力を独立expectedとして使用します。`coverage-analysis`は既存入力契約どおり分析モードと対象範囲を意味判断してcanonical `analysis_scopes[]`へ正規化し、`traceability.py`が各scopeのnormalized inputとcurrent structure / materialize machine stateから同じfixed builderを呼んで`expected_runtime_units[] / expected_entities[]`を内部導出します。`qa-workflow`も同様にcanonical `workflow_scopes[]`だけを渡し、`workflow_runtime.py`がexpected集合を内部導出します。`traceability.py / workflow_runtime.py`のどちらにも完成済みexpected配列を入力せず、Agent / LLMがexpected identity一覧を追加・削除したりactual runtime / Entity集合から逆算したりしません。

`qa-workflow`を経由しないSkill単体利用では、最終自己検証の`verify_runtime_evidence`へcanonical normalized Skill inputとcandidate artifactを渡します。TRD / TCD / test-case-designのpartial rerunではscope外EntityがDispositionだけでも、必須`partial_rerun=true`と同一成果物系列の直前artifact全文を渡します。full buildでは`false + null`です。fixed builderがroot期待runtimeを先に導出し、確認済みcurrent structure / adapter parent resultから条件付きdownstream期待runtimeを段階導出します。Machine Entityはcurrent scopeの生成結果と、前回artifactから固定extract / 検証したscope外carry-forward集合から期待identityを導出し、保存予定のruntime block / Entity identity集合と完全一致で検査します。必須itemのmissingまたは未知のextraがある場合は成果物を契約適合済み・完成済みとして扱いません。`workflow_runtime.py`だけを省略検出の唯一の経路にしません。
