...
各Skillは同じ正規化済みsourceから`expected_entity_identities[]`を固定builderで生成し、Machine Entity actual rowの存在を入力にして期待identityを逆算しません。Skill-local validatorや`traceability.py`でexpected identityが必要な場合は同じfixed builder出力を使用しますが、`workflow_runtime.py`へ完成済み`expected_runtime_units[] / expected_entities[]`を連結して渡しません。既存`qa-workflow`が意味判断で確定したcanonical `workflow_scopes[]`を渡し、`workflow_runtime.py`が各scopeのcanonical normalized inputとcurrent structure / materialize machine stateから同じfixed builderを呼んでexpected集合を内部導出します。Agent / LLMがexpected identity一覧を追加・削除せず、actual runtime / Entity集合からも逆算しません。

`qa-workflow`を経由しないSkill単体利用では、最終自己検証の`verify_runtime_evidence`へcanonical normalized Skill inputとcandidate artifactを渡します。fixed builderがroot期待runtimeを先に導出し、確認済みcurrent structure / adapter parent resultから条件付きdownstream期待runtimeを段階導出して、保存予定の`Machine Runtime Input / Result` block identity集合と完全一致で検査します。必須unitのmissingまたは未知のextraがある場合は成果物を契約適合済み・完成済みとして扱いません。`workflow_runtime.py`だけをruntime省略検出の唯一の経路にしません。
