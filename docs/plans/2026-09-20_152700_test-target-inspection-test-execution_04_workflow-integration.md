...
案件固有のテスト対象資料とARIA snapshot証跡は、ユーザーまたは案件が指定した保存先へだけ永続化します。ARIA snapshotは後続比較に価値があり、安全かつ現在の実行手段で安定した形式を取得できる場合だけ保存し、全画面・全状態の必須成果物にはしません。

`test-target-inspection`は次を返します。

- 保存済み成果物参照
- 構造証跡参照（安全に保存したARIA snapshotがある場合）
- 今回確認した対象範囲
- version / build
- 確認条件
- 変更有無
...
前回と今回のARIA snapshotに差分があっても、その差分だけでは仕様やTCの意味変更とみなしません。`test-target-inspection`が差分を実対象で確認し、画面 / UI要素 / 状態 / ふるまいの意味上の変更として本体成果物へ反映した場合だけ、その変更内容に応じて既存の変更伝播を適用します。snapshot差分から仕様、TC、E2E実装等を直接自動更新しません。
...
永続更新が要求成果物なら`保存結果=保存済み`を確認するまで完了にしません。`保存中止 / 保存失敗`は要求成果物未達として対応範囲を`ブロック中`にします。
