...
1つの`test-execution`成果物では1つのTC入力元 / snapshotを扱います。入力側に既存TC IDまたは外部システムの一意識別子がある場合はその値をTC参照として使います。一意識別子がない外部 / ユーザー直接入力TCは、今回snapshot内だけで一意な成果物ローカル参照を使って追跡し、正式TC IDを新規採番しません。
...
- Playwright Libraryを直接利用し、repoのPlaywright test runnerを起動しない
...
4. 対象環境、URL / origin、role / アカウント、認証、テストデータ、対象version / build等の実施条件を確認する
...
17. 対象version / build、role、viewport、locale、feature flag、テストデータ等が実行途中で変わり、判定への影響を否定できない場合は、残りTCを同じ実行条件として続行しない
...
cleanup失敗は確定済みTC結果を自動でFAILへ変更しません。ただし残存状態がある場合は実行報告とworkflow完了状態へ反映します。次TCの開始状態へ影響する可能性がある場合は次TC開始前に再確認し、安全に開始状態を成立させられないTCだけ`未実行`とします。
...
対象version / build、role、viewport、locale、feature flag、テストデータ等が途中で変わり、TC判定への影響を否定できない場合は、変更後の未開始TCを同じ実行条件の成果物へ追加しません。変更前の確定結果は当時の条件とともに保持し、残りは必要なcleanup後に別成果物 / versionで実行します。version / buildを取得できないことだけで一律に失敗させず、取得不能であることと代替の実施条件を記録します。
...
| TC参照 | 開始状態 / テストデータ | 副作用scope | TC事後状態 / 後処理 | 確認結果 |
| --- | --- | --- | --- | --- |

### 副作用上限

| 副作用scope | 最大回数 | 累計実施回数 | 残数 / 状態 | 根拠 |
| --- | ---: | ---: | --- | --- |
