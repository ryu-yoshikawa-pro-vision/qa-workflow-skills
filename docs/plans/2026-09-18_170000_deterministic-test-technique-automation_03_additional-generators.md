...
- bounded integerは次の`bounded_uint32(bound)`だけを使用し、`1 <= bound <= 2^32`とする
  1. `threshold = (2^32 - bound) mod bound`
  2. `r = next_uint32()`
  3. `r < threshold`なら2へ戻る
  4. `r mod bound`を返す
- `bound=2^32`では`threshold=0`となり、raw uint32をそのまま返す。単純な`next_uint32() % bound`へ置き換えない

seed=`42`の最初の6 raw outputは`2707161783, 2068313097, 3122475824, 2211639955, 3215226955, 3421331566`とし、固定test vectorに使用します。

bounded integerの固定vectorもPlan上で固定します。

- seed=`42`, bound=`10`: threshold=`6`、最初の4結果は`3, 7, 4, 5`
- seed=`42`, bound=`2147483649`: threshold=`2147483647`、最初の4結果は`559678134, 974992175, 64156306, 1067743306`
  - 2件目ではraw `2068313097`をthreshold未満としてrejectし、次のraw `3122475824`をacceptして`974992175`を返す

これらを契約testへ直接使用し、実装結果からexpected vectorを生成しません。
...
JSON pathはfull JSONPathを実装せず、runtime-v1では次の最小文法だけを許可します。

- rootは`# テスト分析・テスト技法の決定論的自動化Plan
...
- object key segmentは`.<identifier>`
- `identifier`は`[A-Za-z_][A-Za-z0-9_-]*`
- array index segmentは`[<index>]`
- `index`は`0|[1-9][0-9]*`
- `# テスト分析・テスト技法の決定論的自動化Plan
...
単独もroot value参照として許可する
- 例: `$.amount`, `$.items[0].price`, `$[0]`
- bracket string key、quoted key、escape、wildcard、filter、slice、recursive descentは実装しない。object keyが上記identifierで表現できない場合やindexが範囲外の場合は`unsupported_path`
- transformの`path`とexpected relationの`output_path`は同じ文法を使う

path parserはこの文法だけを左から順にtokenizeし、独自escapeや別表記を追加しません。
