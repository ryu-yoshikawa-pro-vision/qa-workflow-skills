...
各 `robocopy` の直後に次の形で判定する。

```bat
robocopy ... /MIR /R:0 /W:0
if errorlevel 8 (
    exit /b 1
)
```
...
途中で失敗した場合、それ以前に正常同期されたSkillは同期済みのままとする。rollbackは行わない。原因を解消した後にバッチ全体を再実行し、各Skillを同期元の状態へ収束させる。
...
- 途中失敗時はそれ以前の正常同期済みSkillをrollbackせず、再実行によって再同期すること
...
- コピー失敗時に長時間の再試行を行わない。
- 途中失敗時は正常同期済みSkillをrollbackせず、原因解消後の再実行で同期を完了できる。
- 空白を含むWindowsパスで動作する。
- Skill追加時にバッチ側のSkill一覧修正を必要としない。
- READMEから設定方法、上書き範囲、失敗時の再実行方法を判断できる。
